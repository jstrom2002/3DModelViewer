...
namespace TDModelView 
{
		...
		{
			res = true;
			eng->ui->showFileDialog = false;
			std::string str = m_CurrentPath.string();		
			eng->ui->fileDialogPath = str;
		}
			...
			std::string startingDirectory, ImGuiWindowFlags vFlags) 
		{
			...
			eng->ui->showFileDialog = true;// set this boolean extern value for use in the rest of the program.
		...
		bool CustomFileDialog::FileDialog(const std::string& vKey, ImGuiWindowFlags vFlags) {
			...
			if (m_ShowDialog && dlg_key == vKey) 
			{
				...
				IsOk = false;
				ImGui::SetNextWindowSize(eng->ui->fileDialogSize, ImGuiCond_Always);
					...
					&m_ShowDialog, vFlags | ImGuiWindowFlags_NoScrollbar))
				{
					...
					windowSize = eng->ui->fileDialogSize;
					...
					// 'Home' button.
					if (ImGui::ImageButton(eng->ui->homeID, fileDialogButtonSz))
					{
					...
					// 'Up' button.
					ImGui::SameLine();
					if (ImGui::ImageButton(eng->ui->upArrowID, fileDialogButtonSz))
					{
